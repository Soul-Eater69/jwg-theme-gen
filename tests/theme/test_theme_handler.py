...
def _vs_worklet(vs_id):
    ...
    # carries the VS id (the catalogue lookup key); title + valueStreamId build the theme's
    # businessValueStream ("<title> {<valueStreamId>}").
    return _Worklet(
        id=f"vswlet-{vs_id}",
        source_id="t1",
        properties=[
...
def test_produces_one_theme_per_value_stream():
    ...
    # businessValueStream is built from each VS worklet's title + valueStreamId
